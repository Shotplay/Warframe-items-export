  ...
  for (const arr of itemsArr) {
    ...
    await Promise.all(
      arr.map(async (data, indexMap) => {
        ...
        if (csvDatas.some((dataDatasArr) => dataDatasArr.some((dataDatas) => dataDatas.uniqueName === obj.uniqueName))) return;
  ...
  switch (typeReturn) {
    ...
    case "SQL": {
      ...
      const insertions = flatted.map(
        (data) =>
          `INSERT INTO \`warframeLocalizations\`(\`${Object.keys(data).join("`,`")}\`) VALUES ('${(Object.values(data) as string[])
